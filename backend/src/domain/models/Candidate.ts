  ...
  async save() {
    ...
    if (this.education.length > 0) {
      candidateData.educations = {
        create: this.education.map((edu) => new Education(edu)),
      };
    }
    ...
    if (this.workExperience.length > 0) {
      candidateData.workExperiences = {
        create: this.workExperience.map((exp) => new WorkExperience(exp)),
      };
    }
    ...
    if (this.resumes.length > 0) {
      candidateData.resumes = {
        create: this.resumes.map((resume) => new Resume(resume)),
      };
    }
